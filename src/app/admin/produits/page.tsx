...
import { Plus, Edit, Trash2, Search, Upload, X, Image as ImageIcon, RefreshCw } from "lucide-react";
...
export default function AdminProductsPage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const record = { name: form.name, name_ht: form.name_ht || form.name, description: form.description, description_ht: form.description_ht || form.description, price: Number(form.price), old_price: form.old_price ? Number(form.old_price) : null, category_id: form.category_id || null, gender: form.gender, age_range: form.age_range, stock: Number(form.stock) || 0, badge: form.badge || null, is_coup_coeur: form.is_coup_coeur, colors: form.colors ? form.colors.split(",").map((c) => c.trim()).filter(Boolean) : [], sizes: form.sizes ? form.sizes.split(",").map((s) => s.trim()).filter(Boolean) : [], images: imageUrls, is_active: true };
    try {
      if (editing) { const { error } = await supabase.from("products").update({ ...record, updated_at: new Date().toISOString() }).eq("id", editing.id); if (error) throw error; toast.success("Produit modifié !"); }
      else { const { error } = await supabase.from("products").insert(record); if (error) throw error; toast.success("Produit créé !"); }
  ...
  const handleDelete = async (id: string) => {
    ...
    try { const { error } = await supabase.from("products").delete().eq("id", id); if (error) throw error; toast.success("Supprimé"); await loadProducts(); }
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={loadProducts} className="p-2.5 border-2 border-gray-100 rounded-full text-gray-400 hover:text-rose-dark transition-colors flex items-center justify-center" title="Rafraîchir"><RefreshCw size={16} /></button>
          <button onClick={openNew} className="bg-gradient-to-r from-rose-poudre to-rose-dark text-white font-display font-semibold px-5 py-2.5 rounded-full text-sm flex items-center gap-2 shadow-md justify-center"><Plus size={16} /> Ajouter</button>
        </div>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <h3 className="font-display font-bold text-lg text-gray-600 mb-5">{editing ? "Modifier" : "Nouveau produit"}</h3>
            <div className="mb-5">
              <label className="font-display font-semibold text-xs text-gray-400 uppercase tracking-wide mb-2 block">Photos *</label>
              {imageUrls.length > 0 && <div className="flex gap-2 flex-wrap mb-3">{imageUrls.map((url, i) => (<div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border-2 border-gray-100 group"><img src={url} alt="" className="w-full h-full object-cover" /><button onClick={() => setImageUrls((p) => p.filter((_, j) => j !== i))} className="absolute top-0.5 right-0.5 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><X size={12} /></button>{i === 0 && <span className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-[9px] text-center py-0.5">Principal</span>}</div>))}</div>}
              <input ref={fileRef} type="file" accept="image/*" multiple onChange={handleUpload} className="hidden" />
              <button onClick={() => fileRef.current?.click()} disabled={uploading} className="w-full py-4 border-2 border-dashed border-gray-200 rounded-xl text-sm text-gray-400 hover:border-rose-poudre hover:text-rose-dark transition-colors flex items-center justify-center gap-2 disabled:opacity-50">{uploading ? "Envoi..." : <><Upload size={18} />{imageUrls.length === 0 ? "Ajouter des photos" : "Ajouter plus"}</>}</button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <label className="flex items-center gap-2 text-sm text-gray-500 cursor-pointer"><input type="checkbox" checked={form.is_coup_coeur} onChange={(e) => setForm({ ...form, is_coup_coeur: e.target.checked })} className="w-4 h-4 accent-rose-dark" /> 💝 Coup de cœur</label>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={handleSave} disabled={saving} className="flex-1 bg-gradient-to-r from-rose-poudre to-rose-dark text-white font-display font-semibold py-3 rounded-full flex items-center justify-center disabled:opacity-50">{saving ? "..." : editing ? "💾 Enregistrer" : "✨ Créer"}</button>
      ...
      {loading ? <div className="text-center py-20 text-gray-400"><RefreshCw size={24} className="mx-auto mb-3 animate-spin opacity-40" />Chargement...</div> : (
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100"><tr><th className="text-left px-4 py-3 font-display font-semibold text-gray-400 text-xs">Produit</th><th className="text-left px-4 py-3 font-display font-semibold text-gray-400 text-xs hidden sm:table-cell">Catégorie</th><th className="text-left px-4 py-3 font-display font-semibold text-gray-400 text-xs">Prix</th><th className="text-left px-4 py-3 font-display font-semibold text-gray-400 text-xs hidden md:table-cell">Stock</th><th className="text-right px-4 py-3 font-display font-semibold text-gray-400 text-xs">Actions</th></tr></thead>
          <tbody>{filtered.map((p) => { const cat = DEMO_CATEGORIES.find((c) => c.id === p.category_id); return (
            <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50">
              <td className="px-4 py-3"><div className="flex items-center gap-3">{p.images?.[0] && <img src={p.images[0]} alt="" className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />}<span className="font-display font-semibold text-gray-600 truncate max-w-[150px]">{p.name}</span></div></td>
              ...
              <td className="px-4 py-3 text-right"><div className="flex justify-end gap-1"><button onClick={() => openEdit(p)} className="p-2 text-gray-400 hover:text-bleu-ciel transition-colors flex items-center justify-center"><Edit size={16} /></button><button onClick={() => handleDelete(p.id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors flex items-center justify-center"><Trash2 size={16} /></button></div></td>
            </tr>); })}</tbody>
        </table></div>
        {filtered.length === 0 && <div className="text-center py-10 text-gray-400 text-sm"><ImageIcon size={32} className="mx-auto mb-2 opacity-30" />Aucun produit. Cliquez &quot;Ajouter&quot;.</div>}
        </div>
      )}
    </div>
  );
